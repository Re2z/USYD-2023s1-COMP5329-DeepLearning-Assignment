...
class MLP2:
    ...
    def fit(self, X, y, learning_rate=0.1, epochs=100, mini_batch_size=128, optimizer=None, weight_decay_lambda=1,
        ...
        for k in range(epochs):
            ...
            # Predict the accuracy of the training and validation data for one epoch
            train_pre = nn_barch_norm.predict(X_train)
            train_acc = accuracy_score(y_train, train_pre)
            val_pre = nn_barch_norm.predict(X_val)
            val_acc = accuracy_score(y_val, val_pre)
    ...
    def predict(self, x):
        ...
        pred = np.zeros(x.shape[0])
        # Put all data to the forward progress
        output = nn_barch_norm.forward(x[:, :], test_flg=True)
        # Softmax progress
        exps = np.exp(output - np.max(output, axis=1, keepdims=True))
        output = exps / np.sum(exps, axis=1, keepdims=True)
        # Select the max one as the label
...
nn_barch_norm = MLP2([128, 1280, 640, 320, 120, 32, 10], [None, 'relu', 'relu', 'relu', 'relu', 'relu', 'relu'],
                    dropout=True,
                    batch_norm=False)
...
loss_barch_norm, train_acc_barch_norm, validata_acc_barch_norm = nn_barch_norm.fit(X_train, y_train,
...
plt.plot(loss_barch_norm, label='loss_without_batch_norm')
plt.legend()
plt.show()
...
plt.plot(validata_acc, label='val_acc')
plt.plot(train_acc_barch_norm, label='train_acc_without_batch_norm')
plt.plot(validata_acc_barch_norm, label='val_acc_without_batch_norm')
plt.legend()
plt.show()
...
# Calculate the accuracy of the test dataset
_pre_barch_norm = nn_barch_norm.predict(X_test)
acc_barch_norm = accuracy_score(y_test, _pre_barch_norm)
f1_barch_norm = f1_score(y_test, _pre_barch_norm, average='macro')
recall_barch_norm = recall_score(y_test, _pre_barch_norm, average='macro')
precision_barch_norm = precision_score(y_test, _pre_barch_norm, average='macro')
print('Test-accuracy-batch_norm: {0:.4f}, F1_score: {1:.4f}, Recall: {2:.4f}, Precision: {3:.4f}'.format(acc_barch_norm,
                                                                                                         f1_barch_norm,
                                                                                                         recall_barch_norm,
                                                                                                         precision_barch_norm))
