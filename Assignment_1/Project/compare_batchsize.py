...
class MLP2:
    ...
    def fit(self, X, y, learning_rate=0.1, epochs=100, mini_batch_size=128, optimizer=None, weight_decay_lambda=1,
        ...
        for k in range(epochs):
            ...
            # Predict the accuracy of the training and validation data for one epoch
            train_pre = nn128.predict(X_train)
            train_acc = accuracy_score(y_train, train_pre)
            val_pre = nn128.predict(X_val)
            val_acc = accuracy_score(y_val, val_pre)
    ...
    def predict(self, x):
        ...
        pred = np.zeros(x.shape[0])
        # Put all data to the forward progress
        output = nn128.forward(x[:, :], test_flg=True)
        # Softmax progress
        exps = np.exp(output - np.max(output, axis=1, keepdims=True))
        output = exps / np.sum(exps, axis=1, keepdims=True)
        # Select the max one as the label
...
nn = MLP([128, 1280, 640, 320, 120, 32, 10], [None, 'relu', 'relu', 'relu', 'relu', 'relu', 'relu'],
         dropout=True,
         batch_norm=True)
loss, train_acc, validata_acc = nn.fit(X_train, y_train,
...
nn128 = MLP2([128, 1280, 640, 320, 120, 32, 10], [None, 'relu', 'relu', 'relu', 'relu', 'relu', 'relu'],
         dropout=True,
         batch_norm=True)
loss128, train_acc128, validata_acc128 = nn128.fit(X_train, y_train, learning_rate=0.001, epochs=100, mini_batch_size=128,
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.plot(loss, label='loss_batch size_64')
plt.plot(loss128, label='loss_batch size_128')
plt.legend()
plt.show()
...
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.plot(train_acc, label='train_acc_batch size_64')
plt.plot(validata_acc, label='val_acc_batch size_64')
plt.plot(train_acc128, label='train_acc_batch size_128')
plt.plot(validata_acc128, label='val_acc_batch size_128')
plt.legend()
plt.show()
...
print('Test-accuracy-64: {0:.4f}, F1_score: {1:.4f}, Recall: {2:.4f}, Precision: {3:.4f}'.format(acc, f1, recall,
                                                                                              precision))

# Calculate the accuracy of the test dataset
_pre128 = nn128.predict(X_test)
...
print('Test-accuracy-128: {0:.4f}, F1_score: {1:.4f}, Recall: {2:.4f}, Precision: {3:.4f}'.format(acc_128,
                                                                                                         f1_128,
                                                                                                         recall_128,
                                                                                                         precision_128))
